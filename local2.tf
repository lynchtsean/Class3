locals {
  ...
  # 1. Reverse the entire sentence (simple character reversal)
  ...
  # 3. Count how many vowels (not natively possible, recommend external script)

  # 4. Replace spaces with underscores
  underscore_4 = replace(local.sentence_4, " ", "_")

  # 5. Get the 3rd word (index = 2)
  third_word_5 = element(split(" ", local.sentence_5), 2)

  # 6. Remove punctuation (basic example with chained replace)
  cleaned_6 = replace(
    replace(
      replace(
        replace(
  ...
  # 7. Even-indexed characters only (not supported natively — external script needed)

  # 8. Sentence reversed word-by-word
  reversed_words_8 = join(" ", reverse(split(" ", local.sentence_8)))

  # 9. Capitalize each word (not native in Terraform — use external or manual mapping)

  # 10. Title case except for "in", "of", "at" (use external or lookup map)
